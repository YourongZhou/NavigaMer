...
v31 parts, each capped by `--shard-windows`. Reference slices overlap only
...
| `include/index_persistence.hpp`, `src/index_persistence.cpp` | Array-format v31 binary persistence and manifest signatures |
...
Format v31 loads the finalized arrays directly; older files must be rebuilt.
A `.navshard` bundle points to independently loadable v31 parts and, when it
