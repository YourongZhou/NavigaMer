...
every exact sequence-to-beacon edit distance fits in 8 bits. Format version 14
...
array.
Finalized node, edge, beacon, MBB, and leaf arrays are aligned in the index and
loaded through read-only memory mappings, so index load does not duplicate the
arrays into heap memory. Query access still uses cached raw pointers and sizes.
