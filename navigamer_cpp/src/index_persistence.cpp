...
constexpr std::array<char, 8> kMagic = {'N', 'G', 'I', 'D', 'X', '0', '0', '8'};
...
std::vector<uint8_t> read_u8_vector(
  ...
  if (values.empty()) return values;
  const size_t byte_count = count * sizeof(uint8_t);
  in.read(reinterpret_cast<char*>(values.data()),
          static_cast<std::streamsize>(byte_count));
  if (!in) {
    throw std::runtime_error(std::string("failed to read index field: ") +
...
void write_search_graph_view(std::ostream& out,
  ...
  write_u8_vector(out, view.mbb_lo);
  write_u8_vector(out, view.mbb_hi);
...
SearchGraphView read_search_graph_view(std::istream& in) {
  ...
  view.mbb_lo = read_u8_vector(in, "mbb_lo");
  view.mbb_hi = read_u8_vector(in, "mbb_hi");
