...
void usage(const char* prog) {
  std::cerr << "Usage:\n"
            ...
            << "Global adaptive-search flags: [--mbb-filter-mode scan|rect] [--visited-mode string|epoch] [--graph-view original|flat] [--simd-mode auto|scalar|avx2|avx512] [--distance-mode dp|myers|edlib|auto] [--build-distance-mode dp|edlib|auto] [--search-prefetch off|on] [--search-qgram-prefilter off|on] [--search-qgram-q 5]\n";
}
...
void run_query_on_builder(const navigamer::BioGeometryIndexBuilder& builder,
  ...
  if (mode == "greedy") {
    ...
    std::cout << "Adaptive hits: " << res.size() << " (dist_calcs=" << st.dist_calc_count
              ...
              << " query_path_class=" << st.query_path_class()
              << " path_contained_step_count="
              << st.path_contained_step_count
              << " path_overlap_step_count=" << st.path_overlap_step_count
              << " path_uncovered_step_count="
              << st.path_uncovered_step_count
...
void run_benchmark(const std::string& ref_input, const std::string& query_input,
  ...
  std::vector<std::string> columns = {
      ...
      "path_uncovered_step_count", "mbb_scalar_checks",
...
void run_layer_radius_experiment(const std::string& ref_input,
  ...
  std::vector<std::string> columns = {
      ...
      "candidate_count", "candidate_verify_count", "result_count",
      "source_recovered", "no_fn"};
  std::vector<std::vector<std::string>> rows;
