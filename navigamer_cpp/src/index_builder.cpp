...
constexpr size_t kMaxBuildBeaconsPerNode = 3;
...
bool BioGeometryIndexBuilder::validate_integer_ids() const {
  ...
  if (view.node_records.finest_node_begin() != finest_begin ||
      !view.child_base_ids_valid(finest_begin)) {
    return false;
  }
  for (NodeId node_id = 0; node_id < view.node_records.size(); ++node_id) {
    ...
    if (node_id < finest_begin) {
      ...
      const size_t child_begin = view.child_begin(node_id, node);
      ...
      switch (node.link_storage()) {
        ...
        case WorldNodeRecord::LinkStorage::PackedDelta:
          if (node.packed_leaf_bits() > 16 ||
              view.leaf_link_begin(node_id, node) >
                  view.leaf_id_deltas8.size() ||
      ...
      if (node.link_storage() !=
              WorldNodeRecord::LinkStorage::PackedDelta &&
          static_cast<size_t>(view.leaf_link_begin(node_id, node)) +
                  link_count >
              leaf_array_size) {
        return false;
      }
      for (uint32_t offset = 0; offset < link_count; ++offset) {
        if (view.leaf_id(node_id, node, center_id, offset) >=
            sequence_count_) {
          return false;
        }
      }
...
bool BioGeometryIndexBuilder::validate_search_graph_view() const {
  ...
  if (!view.leaf_link_begins_valid() ||
      (leaf_layout.has_implicit_link_begin() &&
       !leaf_layout.has_implicit_packed_link_fields())) {
    return false;
  }
  ...
  for (NodeId node_id = 0; node_id < view.node_records.size();
    ...
    if (is_finest) {
      if (view.implicit_consecutive_leaf_ids) {
        if (node.link_storage() !=
            WorldNodeRecord::LinkStorage::PackedDelta) {
          return false;
        }
        const LeafId begin =
            center_id > view.implicit_consecutive_leaf_radius
                ? center_id - view.implicit_consecutive_leaf_radius
                : 0;
        const uint64_t end = std::min<uint64_t>(
            sequence_count_, static_cast<uint64_t>(center_id) +
                                 view.implicit_consecutive_leaf_radius + 1);
        if (link_count != end - begin) return false;
      }
      ...
      if (!expected_leaf_begin ||
          view.leaf_link_begin(node_id, node) != *expected_leaf_begin ||
      ...
      *expected_leaf_begin +=
      ...
      if (!expected_child_begin ||
          view.child_begin(node_id, node) != *expected_child_begin) {
        return false;
      }
      if (node.link_storage() ==
          WorldNodeRecord::LinkStorage::PackedDelta) {
...
#pragma omp for schedule(dynamic, 8)
  ...
  size_t ordinary_leaf_beacon_bytes = 0;
  std::array<uint8_t, 3> dense_leaf_mbb_values{};
  size_t dense_leaf_mbb_value_count = 0;
  bool dense_leaf_mbb_ternary = !finest_layer.empty();
  for (NodeId node_id : finest_layer) {
    const auto& node = build_nodes_[node_id];
    const auto& geometry = build_node_geometry_[node.geometry_index];
    if (node.child_or_leaf_ids.size() > 5) {
      dense_leaf_mbb_ternary = false;
    }
    uint32_t maximum = 0;
    for (uint8_t distance : geometry.link_beacon_dists) {
      maximum = std::max(maximum, static_cast<uint32_t>(distance));
      size_t value_idx = 0;
      while (value_idx < dense_leaf_mbb_value_count &&
             dense_leaf_mbb_values[value_idx] != distance) {
        ++value_idx;
      }
      if (value_idx == dense_leaf_mbb_value_count) {
        if (dense_leaf_mbb_value_count == dense_leaf_mbb_values.size()) {
          dense_leaf_mbb_ternary = false;
        } else {
          dense_leaf_mbb_values[dense_leaf_mbb_value_count++] = distance;
        }
      }
    }
    uint8_t ordinary_bits = 1;
    while (maximum >>= 1) ++ordinary_bits;
    ordinary_leaf_beacon_bytes += static_cast<size_t>(
        (static_cast<uint64_t>(node.child_or_leaf_ids.size()) *
             ordinary_bits +
         7) /
        8);
  }
  dense_leaf_mbb_ternary =
      dense_leaf_mbb_ternary && dense_leaf_mbb_value_count != 0 &&
      finest_layer.size() < ordinary_leaf_beacon_bytes;
  if (dense_leaf_mbb_ternary) {
    for (size_t value_idx = dense_leaf_mbb_value_count;
         value_idx < dense_leaf_mbb_values.size(); ++value_idx) {
      dense_leaf_mbb_values[value_idx] = dense_leaf_mbb_values[0];
    }
  }
  for (NodeId node_id : finest_layer) {
    auto& node = build_nodes_[node_id];
    ...
    if (dense_leaf_mbb_ternary) {
      ...
      for (uint8_t distance : geometry.link_beacon_dists) {
        const auto value = std::find(
            dense_leaf_mbb_values.begin(), dense_leaf_mbb_values.end(),
            distance);
        if (value == dense_leaf_mbb_values.end()) {
          throw std::runtime_error("dense ternary leaf MBB value is missing");
        }
        packed = static_cast<uint8_t>(
            packed + static_cast<uint8_t>(value - dense_leaf_mbb_values.begin()) *
                         multiplier);
        multiplier = static_cast<uint8_t>(multiplier * 3);
      }
      packed_distances.assign(1, packed);
      build_geometry_mbb_bits_[node.geometry_index] = 1;
    } else {
      build_geometry_mbb_bits_[node.geometry_index] =
  ...
  if (!finest_layer.empty() && sequence_count_ != 0) {
    size_t maximum_leaf_count = 0;
    for (NodeId node_id : finest_layer) {
      maximum_leaf_count = std::max(
          maximum_leaf_count, build_nodes_[node_id].child_or_leaf_ids.size());
    }
    if (maximum_leaf_count != 0 && (maximum_leaf_count & 1U) != 0 &&
        static_cast<uint64_t>(maximum_leaf_count) <=
            static_cast<uint64_t>(std::numeric_limits<LeafId>::max()) * 2 +
                1) {
      const LeafId radius = static_cast<LeafId>(
          (maximum_leaf_count - 1) / 2);
      bool exact_consecutive_intervals = true;
      for (NodeId node_id : finest_layer) {
        const auto& node = build_nodes_[node_id];
        const LeafId begin =
            node.center_sequence_id > radius
                ? node.center_sequence_id - radius
                : 0;
        const uint64_t end = std::min<uint64_t>(
            sequence_count_, static_cast<uint64_t>(node.center_sequence_id) +
                                 radius + 1);
        if (node.child_or_leaf_ids.size() != end - begin) {
          exact_consecutive_intervals = false;
          break;
        }
        for (size_t leaf_offset = 0;
             leaf_offset < node.child_or_leaf_ids.size(); ++leaf_offset) {
          if (node.child_or_leaf_ids[leaf_offset] !=
              static_cast<LeafId>(begin + leaf_offset)) {
            exact_consecutive_intervals = false;
            break;
          }
        }
        if (!exact_consecutive_intervals) break;
      }
...
void BioGeometryIndexBuilder::build_search_graph_view() {
  ...
  bool implicit_contiguous_child_ranges =
      !primary_layers_.empty() &&
      primary_layers_.back().size() < world_node_count_ &&
      child_base_bytes != 0;
  if (implicit_contiguous_child_ranges) {
    for (size_t layer_idx = 0;
         layer_idx + 1 < primary_layers_.size(); ++layer_idx) {
      for (NodeId node_id : primary_layers_[layer_idx]) {
        const auto choice = child_storage_for(node_id);
        if (choice.storage != WorldNodeRecord::LinkStorage::PackedDelta ||
            choice.packed_bits !=
                SearchGraphView::CONTIGUOUS_CHILD_RANGE_BITS) {
          implicit_contiguous_child_ranges = false;
          break;
        }
      }
      if (!implicit_contiguous_child_ranges) break;
    }
  }
  bool implicit_leaf_packed_fields =
      !primary_layers_.empty() && !primary_layers_.back().empty();
  uint8_t implicit_leaf_packed_bits = 0;
  if (implicit_leaf_packed_fields) {
    for (NodeId node_id : primary_layers_.back()) {
      const auto choice = leaf_storage_for(node_id);
      if (choice.storage != WorldNodeRecord::LinkStorage::PackedDelta) {
        implicit_leaf_packed_fields = false;
        break;
      }
      implicit_leaf_packed_bits = std::max(
          implicit_leaf_packed_bits, choice.packed_bits);
    }
  }
  const bool implicit_consecutive_leaf_ids =
      implicit_consecutive_leaf_ids_ && implicit_leaf_packed_fields;
  bool matching_leaf_payload_starts =
      implicit_leaf_packed_fields && !implicit_consecutive_leaf_ids;
  if (matching_leaf_payload_starts) {
    size_t leaf_id_begin = 0;
    size_t leaf_mbb_begin = 0;
    for (NodeId node_id : primary_layers_.back()) {
      const auto& node = build_nodes_[node_id];
      const auto& geometry = build_node_geometry_[node.geometry_index];
      if (leaf_id_begin != leaf_mbb_begin) {
        matching_leaf_payload_starts = false;
        break;
      }
      leaf_id_begin += static_cast<size_t>(
          (static_cast<uint64_t>(node.child_or_leaf_ids.size()) *
               implicit_leaf_packed_bits +
           7) /
          8);
  ...
  const uint32_t finest_node_begin =
      primary_layers_.empty()
          ? to_u32(world_node_count_, "world_node_count")
          : to_u32(
                world_node_count_ - primary_layers_.back().size(),
                "finest_node_begin");
  uint64_t node_count_overflow_count = 0;
  uint64_t maximum_child_link_begin = 0;
  uint64_t maximum_leaf_link_begin = 0;
  uint64_t maximum_child_mbb_begin = 0;
  uint64_t maximum_leaf_mbb_begin = 0;
  uint64_t maximum_child_count_field = 0;
  uint64_t maximum_leaf_count_field = 0;
  ...
  for (size_t layer_idx = 0;
    ...
    for (NodeId build_node_id : layer) {
      ...
      have_previous = true;
      ++node_offset;
      const bool counts_overflow =
          node.child_or_leaf_ids.size() >
              WorldNodeRecord::LINK_COUNT_MASK ||
          (!is_finest && geometry.beacon_ids.size() >=
                             WorldNodeRecord::COUNT_OVERFLOW_CODE);
      const uint64_t count_field =
          counts_overflow
              ? node_count_overflow_count
              : std::min<uint64_t>(
                    node.child_or_leaf_ids.size(),
                    WorldNodeRecord::LINK_COUNT_MASK);
      auto& maximum_count_field =
          is_finest ? maximum_leaf_count_field
                    : maximum_child_count_field;
      maximum_count_field = std::max(
          maximum_count_field, count_field);
      if (counts_overflow) {
        ++node_count_overflow_count;
      }
      uint32_t plan = 0;
      ...
      if (is_finest) {
        ...
        set_plan_bits(
            ...
            packed_bits == 0 ? 1 : packed_bits);
        if (implicit_consecutive_leaf_ids) {
          if (storage != WorldNodeRecord::LinkStorage::PackedDelta) {
            throw std::runtime_error(
                "implicit consecutive leaf IDs require packed leaf storage");
          }
        } else if (storage == WorldNodeRecord::LinkStorage::Delta8) {
          maximum_leaf_link_begin = std::max<uint64_t>(
              maximum_leaf_link_begin, total_leaf_deltas8);
          ...
          maximum_leaf_link_begin = std::max<uint64_t>(
              maximum_leaf_link_begin, total_leaf_deltas16);
          total_leaf_deltas16 += node.child_or_leaf_ids.size();
        } else {
          maximum_leaf_link_begin = std::max<uint64_t>(
              maximum_leaf_link_begin, total_leaf_ids32);
        ...
        if (storage == WorldNodeRecord::LinkStorage::Delta8) {
          ...
          maximum_child_link_begin = std::max<uint64_t>(
              maximum_child_link_begin, total_child_ids32);
  ...
  uint8_t beacon_begin_block_size =
      SearchGraphView::DEFAULT_BEACON_BEGIN_BLOCK_SIZE;
  size_t minimum_beacon_begin_bytes = std::numeric_limits<size_t>::max();
  for (uint8_t candidate_block_size : {uint8_t{2}, uint8_t{4},
    ...
    for (size_t layer_idx = 0;
      ...
      for (NodeId node_id : primary_layers_[layer_idx]) {
        ...
        const auto& geometry =
            build_node_geometry_[node.geometry_index];
        if (node_id % candidate_block_size == 0) {
          block_base = payload_bytes;
        }
        maximum_begin = std::max<uint64_t>(maximum_begin, payload_bytes);
        maximum_delta = std::max<uint64_t>(
            maximum_delta, payload_bytes - block_base);
        const auto storage = beacon_storage_for(
            node.center_sequence_id, geometry.beacon_ids);
        if (storage == WorldNodeRecord::BeaconStorage::Delta8) {
          payload_bytes += geometry.beacon_ids.size();
        } else if (storage == WorldNodeRecord::BeaconStorage::PackedDelta) {
          ...
          payload_bytes += geometry.beacon_ids.size() * sizeof(LeafId);
        }
      }
    }
    if (payload_bytes != total_beacon_id_bytes) {
      throw std::runtime_error("beacon payload sizing is inconsistent");
    }
    const size_t block_count =
        (static_cast<size_t>(finest_node_begin) + candidate_block_size - 1) /
        candidate_block_size;
    const size_t record_bits =
        PackedWorldNodeLayout::bits_for_value(maximum_begin) +
        static_cast<size_t>(candidate_block_size - 1) *
            PackedWorldNodeLayout::bits_for_value(maximum_delta);
    const size_t bytes = block_count * ((record_bits + 7) / 8);
    if (bytes < minimum_beacon_begin_bytes) {
      minimum_beacon_begin_bytes = bytes;
      beacon_begin_block_size = candidate_block_size;
      maximum_beacon_begin = maximum_begin;
      maximum_beacon_block_delta = maximum_delta;
    }
  }
  ...
  if (!implicit_leaf_mbb_offsets && !implicit_consecutive_leaf_ids) {
    implicit_leaf_mbb_offsets =
        !primary_layers_.empty() && !primary_layers_.back().empty();
    for (NodeId node_id : primary_layers_.back()) {
      const auto& node = build_nodes_[node_id];
      const auto& geometry = build_node_geometry_[node.geometry_index];
  ...
  if (implicit_leaf_link_begins) {
    view.initialize_leaf_link_begin_blocks(
        primary_layers_.back().size());
  }
  ...
  for (size_t layer_idx = 0; layer_idx < primary_layers_.size(); ++layer_idx) {
    ...
    for (NodeId build_node_id : layer) {
      ...
      if (!is_finest) {
        ...
        if (link_storage == WorldNodeRecord::LinkStorage::Delta8 ||
          ...
          if (link_storage == WorldNodeRecord::LinkStorage::PackedDelta) {
            record.set_packed_child_layout(
                view.implicit_contiguous_child_ranges ? 0 : byte_begin,
                child_packed_bits);
          } else {
        ...
        record.set_link_storage(link_storage);
        if (link_storage == WorldNodeRecord::LinkStorage::PackedDelta) {
          const uint32_t leaf_begin = to_u32(
              view.leaf_id_deltas8.size(), "leaf_id_deltas8");
          if (implicit_leaf_link_begins &&
              (node_id - view.node_records.finest_node_begin()) %
                      SearchGraphView::LEAF_LINK_BEGIN_BLOCK_SIZE ==
                  0) {
            view.set_leaf_link_begin(node_id, leaf_begin);
          }
          record.set_packed_leaf_layout(
              implicit_leaf_link_begins ? 0 : leaf_begin,
              leaf_packed_bits);
          if (!view.implicit_consecutive_leaf_ids) {
            const size_t byte_count = static_cast<size_t>(
                (static_cast<uint64_t>(
        ...
        for (size_t leaf_offset = 0;
          ...
          if (link_storage == WorldNodeRecord::LinkStorage::Delta8) {
            ...
            view.leaf_id_deltas16.push_back(
              ...
              link_storage ==
              WorldNodeRecord::LinkStorage::PackedDelta) {
            if (view.implicit_consecutive_leaf_ids) continue;
            ...
            const uint32_t shift =
                static_cast<uint32_t>(bit_offset & 7);
            const size_t payload_begin =
                view.leaf_link_begin(node_id, record);
      ...
      if (!is_finest) {
        ...
        if (view.dense_beacon_patterns) {
          ...
          if (node_id % view.beacon_begin_block_size == 0) {
