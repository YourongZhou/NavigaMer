...
usage() {
  cat <<'USAGE'
Usage: scripts/codex_run_until_ecoli_1p1m_strobe_spaced.sh [options]

Run the repository Codex automation loop with the E. coli 1.1M goal:
make NavigaMer faster than strobemer and spaced seed under exact-verified
no-false-negative accounting.

Options are passed through to scripts/codex_autorun.sh after this wrapper's
own setup. Common examples:

  scripts/codex_run_until_ecoli_1p1m_strobe_spaced.sh --sleep 5h10m
  scripts/codex_run_until_ecoli_1p1m_strobe_spaced.sh --sleep 0s --max-attempts 1

This wrapper:
  1. copies CODEX_ECOLI_1P1M_STROBE_SPACED_GOAL.md into CODEX_GOAL.md;
...
GOAL_TEMPLATE="$REPO/CODEX_ECOLI_1P1M_STROBE_SPACED_GOAL.md"
