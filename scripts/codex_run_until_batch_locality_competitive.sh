...
usage() {
  cat <<'USAGE'
Usage: scripts/codex_run_until_batch_locality_competitive.sh [options]

Run the repository Codex automation loop with the scoped batch-locality
competitive-query goal.

Options are passed through to scripts/codex_autorun.sh after this wrapper's
own setup. Common examples:

  scripts/codex_run_until_batch_locality_competitive.sh --sleep 5h10m
  scripts/codex_run_until_batch_locality_competitive.sh --sleep 0s --max-attempts 1

This wrapper:
  1. copies CODEX_BATCH_LOCALITY_GOAL.md into CODEX_GOAL.md;
...
GOAL_TEMPLATE="$REPO/CODEX_BATCH_LOCALITY_GOAL.md"
