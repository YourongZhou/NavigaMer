...
usage() {
  cat <<'USAGE'
Usage: scripts/codex_run_until_ecoli_1p1m_real_locality_v2.sh [options]

Run the repository Codex automation loop with the E. coli 1.1M real-read
locality V2 goal: duplicated real reads plus source-sorted near-repeat reads,
with exact verification and no-false-negative accounting.

Options are passed through to scripts/codex_autorun.sh after this wrapper's
own setup. Common examples:

  scripts/codex_run_until_ecoli_1p1m_real_locality_v2.sh --sleep 5h10m
  scripts/codex_run_until_ecoli_1p1m_real_locality_v2.sh --sleep 0s --max-attempts 1

This wrapper:
  1. copies CODEX_ECOLI_1P1M_REAL_LOCALITY_V2_GOAL.md into CODEX_GOAL.md;
  ...
  3. pins Codex to gpt-5.5 and high reasoning effort unless the caller already
     supplied explicit CODEX_* overrides; service tier is left at the Codex
     default unless CODEX_SERVICE_TIER is explicitly set;
...
GOAL_TEMPLATE="$REPO/CODEX_ECOLI_1P1M_REAL_LOCALITY_V2_GOAL.md"
