...
usage() {
  cat <<'USAGE'
Usage: scripts/codex_run_until_fast_no_fn.sh [options]

Run the repository Codex automation loop with the strict "faster than baseline
and no false negatives" goal.

Options are passed through to scripts/codex_autorun.sh after this wrapper's
own setup. Common examples:

  scripts/codex_run_until_fast_no_fn.sh --sleep 5h10m
  scripts/codex_run_until_fast_no_fn.sh --sleep 0s --max-attempts 1

This wrapper:
  1. copies CODEX_FAST_NO_FN_GOAL.md into CODEX_GOAL.md so resume prompts use
     the strict goal;
...
GOAL_TEMPLATE="$REPO/CODEX_FAST_NO_FN_GOAL.md"
