...
usage() {
  cat <<'USAGE'
Usage: scripts/codex_run_until_ecoli_result1to6_overnight.sh [options]

Run the repository Codex automation loop with the overnight Result 1-6
gap-fill goal. Result 3 is explicitly restricted to a 10K E. coli prefix; the
other experiments use the 1.1M reference or existing 1.1M geom index when
practical.

Options are passed through to scripts/codex_autorun.sh after this wrapper's
own setup. Common examples:

  scripts/codex_run_until_ecoli_result1to6_overnight.sh --sleep 5h
  scripts/codex_run_until_ecoli_result1to6_overnight.sh --sleep 0s --max-attempts 1

This wrapper:
  1. copies CODEX_ECOLI_RESULT1TO6_OVERNIGHT_GOAL.md into CODEX_GOAL.md;
...
GOAL_TEMPLATE="$REPO/CODEX_ECOLI_RESULT1TO6_OVERNIGHT_GOAL.md"
