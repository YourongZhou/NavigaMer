...
SpacedSeedIndex SpacedSeedIndex::load(
    const std::filesystem::path& index_path) {
  return load(read_index_file(index_path));
}

SpacedSeedIndex SpacedSeedIndex::load(const PersistedIndex& loaded_index) {
  if (loaded_index.manifest.method != "spaced") {
    throw std::runtime_error("unsupported spaced index method");
  }

  SpacedSeedIndex index;
  index.manifest_ = loaded_index.manifest;
  const auto [weight, packed_payload] = parse_spaced_payload(
      loaded_index.payload);
  (void)packed_payload;
